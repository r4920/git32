...
const softDeleteManyChat_group = async (req, res) => {
  try {
    ...
    if (ids){
      ...
      const options = {};
      let result = await dbService.softDeleteMany(Chat_group,query,updateBody, options);
      if (!result) {
        return res.recordNotFound();
      }
...
const deleteManyChat_group = async (req, res) => {
  try {
    ...
    let deletedChat_group = await dbService.deleteMany(Chat_group,query);
...
const deleteChat_group = async (req, res) => {
  try {
    const result = await dbService.deleteByPk(Chat_group, req.params.id);
    if (result){
      return  res.success({ data :result });
    }
    return res.recordNotFound();
