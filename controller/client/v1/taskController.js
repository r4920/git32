...
const softDeleteManyTask = async (req, res) => {
  try {
    ...
    let updatedTask = await deleteDependentService.softDeleteTask(query, updateBody);
    if (!updatedTask) {
      return res.recordNotFound();
    }
...
const deleteManyTask = async (req, res) => {
  try {
    ...
    if (dataToDelete.isWarning){
      let countedTask = await deleteDependentService.countTask(query);
      if (!countedTask) {
        return res.recordNotFound();
      }
      return res.success({ data: countedTask });            
    }
    let deletedTask = await deleteDependentService.deleteTask(query);
    if (!deletedTask) {
      return res.recordNotFound();
    }
